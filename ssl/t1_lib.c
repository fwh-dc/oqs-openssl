                    ...
                    int isec, int *okfortls13)
{
    const TLS_GROUP_INFO *ginfo = tls1_group_id_lookup(SSL_CONNECTION_GET_CTX(s),
                                                       group_id);
    int ret;
    int group_minversion, group_maxversion, tls13_version;
    ...
    tls13_version = SSL_CONNECTION_IS_DTLS(s) ? DTLS1_3_VERSION : TLS1_3_VERSION;
    ...
    if (ret && okfortls13 != NULL && maxversion == tls13_version)
        *okfortls13 = (group_maxversion == 0)
                      || (ssl_version_cmp(s, group_maxversion, tls13_version) >= 0);
